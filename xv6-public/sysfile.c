...
name_of_inode(struct inode *ip, struct inode *parent, char buf[DIRSIZ])
{
    ...
    for (off = 0; off < parent->size; off += sizeof(de)) {
        if (readi(parent, (char*)&de, off, sizeof(de)) != sizeof(de))
            panic("node_of_inode: can't read dir entry");
        if (de.inum == ip->inum) {
...
int
name_4_inode(char* buf, int n, struct inode *ip)
{
    int path_offset;
    struct inode *parent;
    ...
    if (ip->inum == namei("/")->inum) {
        ...
        if (name_of_inode(ip, parent, node_name)) {
            panic("name_4_inode: could not find name of inode in parent!");
        }
        path_offset = name_4_inode(buf, n, parent);
        ...
        panic("name_4_inode: invalid inode type");
    }
}
...
sys_getcwd(void)
{
    ...
    return name_4_inode(p, i, myproc()->cwd);
}
